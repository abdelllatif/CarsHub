...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reservation_id']) && isset($_POST['status'])) {
    error_log("POST received - reservation_id: " . $_POST['reservation_id'] . ", status: " . $_POST['status']);
                        ...
                        if (!$reservations) {
                            ...
                            foreach ($reservations as $reservation) {
                              ...
                              if ($reservation['status'] == 'canceled' || $reservation['status'] == 'refused' || $reservation['status'] == 'en Attend') {
                                ...
                                echo "<button type='submit' class='text-red-600 hover:text-red-900'>cancele</button>";
                                echo "</form>";
                                echo "<span class=\"add-review-btn text-sm text-gray-800 py-2 cursor-pointer\" data-reservation-id=\"" . htmlspecialchars($reservation['id']) . "\">Voir les avis</span>";
                            }
                            }
                        }
                        ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add click event to "Add Review" buttons
